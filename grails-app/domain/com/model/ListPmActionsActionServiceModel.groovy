...
class ListPmActionsActionServiceModel {
    ...
    public static final String SQL_LIST_ACTIONS_MODEL = """
            ...
            CREATE OR REPLACE VIEW list_pm_actions_action_service_model AS
             ...
             SELECT d.id, d.version,d.actions, g.id AS goal_id, g.goal,d.res_person_id,d.sequence, sc.id AS service_id,sc.name AS service,d.indicator,
            ...
            GROUP BY  sc.id,g.id,EXTRACT(YEAR FROM d.start), d.sequence
            ORDER BY EXTRACT(YEAR FROM d.start) , sc.sequence  ,g.id ,d.sequence;
    """
