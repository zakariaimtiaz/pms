...
class EdDashboardService  extends BaseService{
    ...
    public List<GroovyRowResult> lstEdDashboardIssue(long serviceId,Date monthFor) {
        ...
        SELECT  edi.id ,edi.version,edi.issue_name ,ed.description,ed.remarks,ed.ed_advice,edi.is_heading,
            CASE
                WHEN MONTH(DATE_FORMAT('${monthFor}' + INTERVAL 1 MONTH, '%Y-%m-01'))>=MONTH(NOW())
        ...
        LEFT JOIN ed_dashboard ed  ON ed.issue_id=edi.id AND ed.service_id=${serviceId}
                AND MONTH(ed.month_for)=MONTH('${monthFor}') AND YEAR(ed.month_for)=YEAR('${monthFor}')
