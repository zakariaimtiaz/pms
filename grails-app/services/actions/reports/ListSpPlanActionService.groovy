...
class ListSpPlanActionService extends BaseService implements ActionServiceIntf {
        ...
        try {
            ...
            if(type.equals("Mission")){
                ...
                try{
                    ...
                    List<GroovyRowResult> lstAction = buildGoalWiseActionsList(serviceId,goalId,start,end)
    ...
    private List<GroovyRowResult> buildGoalWiseActionsList(long serviceId,long goalId,Date start, Date end) {
        String query = """
        ...
        (SELECT GROUP_CONCAT(short_name SEPARATOR ', ') FROM pm_projects WHERE LOCATE(CONCAT(',',id,',') ,CONCAT(',',a.source_of_fund,', '))>0 ) sourceOfFundStr
                ...
                WHERE a.service_id = ${serviceId} AND a.goal_id = ${goalId}
