...
@Transactional
class GetDropDownMeetingCategoryTaglibActionService extends BaseService implements ActionServiceIntf {
    ...
    private static final String MEETING_CATEGORY = "Meeting Category"
    ...
    public Map execute(Map result) {
        try {
            String typeStr = result.get(MEETING_TYPE)
            SystemEntity meetingType = SystemEntity.findByNameAndTypeId(typeStr, 5L)
            def loggedUser = springSecurityService.principal
            SecUser user = SecUser.read(loggedUser.id)
            PmServiceSector serviceSector = PmServiceSector.read(user.serviceId)
            List<MeetingCategory> lstCategory = null
            if(typeStr.equals(MONTHLY)){
                lstCategory = MeetingCategory.findAllByMeetingTypeIdAndServiceCatId(meetingType.id,serviceSector.categoryId)
            }
     ...
     *
     * @param lstCategory - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<MeetingCategory> lstCategory, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstCategory = listForKendoDropdown(lstCategory, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstCategory as JSON
        String script = """ \n
