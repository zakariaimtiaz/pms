...
class GetDropDownSystemEntityTypeTaglibActionService extends BaseService implements ActionServiceIntf {
     ...
     *
     * @param lstSystemEntityType - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<SystemEntityType> lstSystemEntityType, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstSystemEntityType = listForKendoDropdown(lstSystemEntityType, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstSystemEntityType as JSON
        String script = """ \n
